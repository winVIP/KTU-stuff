    ...
    public class ChartStyle
    {
        ...
        private void AddLabels(Graphics g)
        {
            ...
            for (float x = XMin + XTick; x < XMax; x = x + XTick)
            {
                ...
                g.DrawString(x.ToString(), TickFont, aBrush, new PointF(pt.X, pt.Y), sf);
            }
            // Add y tick labels:
            offset = (XMax - XMin) / 20;
            labelSpace = offset;
            for (float y = YMin + yTick; y < YMax; y = y + YTick)
            {
                ...
                pt.Transform(m, form1, this);
                g.DrawString(y.ToString(), TickFont, aBrush, new PointF(pt.X, pt.Y), sf);
            }
            // Add z tick labels:
            ...
            for (float z = ZMin; z <= ZMax; z = z + ZTick)
            {
                ...
                pt.Transform(m, form1, this);
                g.DrawString(z.ToString(), TickFont, aBrush, new PointF(pt.X - labelSpace, pt.Y - s.Height / 2), sf);
            }
            // Add Title:
            sf.Alignment = StringAlignment.Center;
            ...
            if (Title != "No Title")
            {
                g.DrawString(Title, titleFont, aBrush,new PointF(form1.PlotPanel.Width / 2, form1.Height / 30), sf);
            }
            aBrush.Dispose();
            // Add x axis label:
            offset = (YMax - YMin) / 3;
            ...
            g.DrawString(XLabel, LabelFont, aBrush,new PointF(0, 0), sf);
            g.Restore(gs);
            // Add y axis label:
            offset = (XMax - XMin) / 3;
            offset1 = (YMax - YMin) / 5;
            ...
            g.TranslateTransform(pta[1].X, pta[1].Y);
            g.RotateTransform(theta);
            g.DrawString(YLabel, LabelFont, aBrush, new PointF(0, 0), sf);
            g.Restore(gs);
            // Add z axis labels:
            ...
            g.DrawString(ZLabel, LabelFont, aBrush, new PointF(0, 0), sf);
            g.Restore(gs);
        }
