    ...
    public class DrawChart
    {
        ...
        public void AddColorBar(Graphics g, DataSeries ds, ChartStyle cs, ChartStyle2D cs2d)
        {
            if (cs.IsColorBar && IsColorMap)
            {
                ...
                if (ChartType == ChartTypeEnum.Contour || ChartType == ChartTypeEnum.FillContour || ChartType == ChartTypeEnum.XYColor)
                {
                ...
                else
                {
                    ...
                    for (float z = zmin; z <= zmax; z = z + (zmax - zmin) / 6)
                    {
                        ...
                        g.DrawString((Math.Round(z, 2)).ToString(),
                        cs.TickFont, aBrush, new PointF(x + width + 5, yy - size.Height / 2), sFormat);
                    }
                }
            }
        }
        ...
        public void AddLine(Graphics g, DataSeries ds, ChartStyle cs)
        {
            ...
            Matrix3 m = Matrix3.AzimuthElevation(
            cs.Elevation, cs.Azimuth);
            ...
            if (ds.LineStyle.IsVisible == true)
            {
                ...
                {
                    g.DrawLine(aPen, pts[i - 1].X,
                    pts[i - 1].Y, pts[i].X, pts[i].Y);
                }
                aPen.Dispose();
            }
        }
        ...
        private void AddMeshZ(Graphics g, DataSeries ds, ChartStyle cs)
        {
            ...
            for (int i = 0; i < pts.GetLength(0) - 1; i++)
            {
                for (int j = 0; j < pts.GetLength(1) - 1; j++)
                {
                    ...
                    pta[0] = new PointF(pts[ii, j].X, pts[ii, j].Y);
                    pta[1] = new PointF(pts[ii, j + 1].X, pts[ii, j + 1].Y);
                    pta[2] = new PointF(pts[ii + 1, j + 1].X,
                    pts[ii + 1, j + 1].Y);
                    pta[3] = new PointF(pts[ii + 1, j].X, pts[ii + 1, j].Y);
                    g.FillPolygon(aBrush, pta);
                    if (IsColorMap)
            ...
            for (int j = 0; j < pts1.GetLength(1); j++)
            {
                ...
                {
                    pt3[0] = new Point3(pts1[ii, j].X, pts1[ii, j].Y,
                    pts1[ii, j].Z, 1);
                    pt3[1] = new Point3(pts1[ii, j + 1].X,
                    pts1[ii, j + 1].Y, pts1[ii, j + 1].Z, 1);
                    pt3[2] = new Point3(pts1[ii, j + 1].X,
                    pts1[ii, j + 1].Y, cs.ZMin, 1);
                    pt3[3] = new Point3(pts1[ii, j].X,
                    pts1[ii, j].Y, cs.ZMin, 1);
                    for (int k = 0; k < 4; k++)
                        pt3[k].Transform(m, form1, cs);
                    pta[0] = new PointF(pt3[0].X, pt3[0].Y);
                    pta[1] = new PointF(pt3[1].X, pt3[1].Y);
...
ChartStyle cs, ChartStyle2D cs2d)
        {
            Pen aPen = new Pen(ds.LineStyle.LineColor,
            ds.LineStyle.Thickness);
            aPen.DashStyle = ds.LineStyle.Pattern;
            ...
            for (int i = 0; i < pts.GetLength(0); i++)
            {
                ...
                {
                    // Make a deep copy the points array:
                    pts1[i, j] = new Point3(pts[i, j].X,
pts[i, j].Y, pts[i, j].Z, 1);
                    // Perform transformation on points:
                    pts[i, j].Transform(m, form1, cs);
                }
            }
        ...
        private void Interp(Graphics g, ChartStyle cs, ChartStyle2D cs2d,
Matrix3 m, Point3[] pta, float zmin, float zmax, int flag)
            ...
            if (flag == 1) // For Surface chart:
            {
                ...
                for (int i = 0; i < npoints; i++)
                {
                    for (int j = 0; j < npoints; j++)
                    {
                        ...
                        points[1] = new PointF(pts[i + 1, j].X,
                        pts[i + 1, j].Y);
                        points[2] = new PointF(pts[i + 1, j + 1].X,
                        pts[i + 1, j + 1].Y);
                        points[3] = new PointF(pts[i, j + 1].X,
                        pts[i, j + 1].Y);
            ...
            else if (flag == 2) // For XYColor chart:
            {
                ...
                for (int i = 0; i < npoints; i++)
                {
                    for (int j = 0; j < npoints; j++)
                    {
                        color = AddColor(cs, pts[i, j], zmin, zmax);
                        aBrush = new SolidBrush(color);
                        points[0] = cs2d.Point2D(new PointF(pts[i, j].X,
                        pts[i, j].Y), cs);
                        points[1] = cs2d.Point2D(new PointF(pts[i + 1, j].X,
                        pts[i + 1, j].Y), cs);
                        points[2] = cs2d.Point2D(new PointF(pts[i + 1,
                        j + 1].X, pts[i + 1, j + 1].Y), cs);
                        points[3] = cs2d.Point2D(new PointF(
                        pts[i, j + 1].X, pts[i, j + 1].Y), cs);
                        g.FillPolygon(aBrush, points);
                        aBrush.Dispose();
                    }
                }
        ...
        private void AddXYColor(Graphics g, DataSeries ds,
ChartStyle cs, ChartStyle2D cs2d)
        {
            ...
            if (!IsInterp)
            {
                for (int i = 0; i < pts.GetLength(0) - 1; i++)
                {
                    for (int j = 0; j < pts.GetLength(1) - 1; j++)
                    {
                        pta[0] = cs2d.Point2D(new PointF(pts[i, j].X,
                        pts[i, j].Y), cs);
                        pta[1] = cs2d.Point2D(new PointF(pts[i, j + 1].X,
                        pts[i, j + 1].Y), cs);
                        pta[2] = cs2d.Point2D(new PointF(pts[i + 1, j + 1].X,
                        pts[i + 1, j + 1].Y), cs);
                        pta[3] = cs2d.Point2D(new PointF(pts[i + 1, j].X,
                        pts[i + 1, j].Y), cs);
                ...
                for (int j = 0; j < pts.GetLength(1) - 1; j++)
                {
                    ...
                    for (int k = 0; k < numberContours; k++)
                    {
                        ...
                        && zlevels[k] >= pts[i2, j2].Z))
                        {
                            ...
                            (1 - zratio) * pts[i0, j0].Y + zratio *
                            pts[i1, j1].Y), cs);
                            zratio = (zlevels[k] - pts[i1, j1].Z) /
                            (pts[i2, j2].Z - pts[i1, j1].Z);
                            pta[1] = cs2d.Point2D(new PointF((1 - zratio) *
                            pts[i1, j1].X + zratio * pts[i2, j2].X,
                            pts[i1, j1].Y), cs);
                            g.DrawLine(aPen, pta[0], pta[1]);
                        }
                        else if ((zlevels[k] >= pts[i0, j0].Z && zlevels[k] < pts[i2, j2].Z || zlevels[k] < pts[i0, j0].Z
&& zlevels[k] >= pts[i2, j2].Z) &&
(zlevels[k] >= pts[i1, j1].Z && zlevels[k] <
pts[i2, j2].Z || zlevels[k] < pts[i1, j1].Z
&& zlevels[k] >= pts[i2, j2].Z))
                        {
                            ...
                            pta[0] = cs2d.Point2D(new PointF((1 - zratio) *
                        ...
                        zlevels[k] >= pts[i2, j2].Z))
                        {
                            ...
                            zratio = (zlevels[k] - pts[i0, j0].Z) /
                            (pts[i2, j2].Z - pts[i0, j0].Z);
                            pta[1] = cs2d.Point2D(new PointF(pts[i0, j0].X *
   ...
   (pts[i1, j1].Z - pts[i0, j0].Z);
                            ...
                            pts[i1, j1].Y * (1 - zratio) + pts[i2, j2].Y
                            * zratio), cs);
                            g.DrawLine(aPen, pta[0], pta[1]);
                        }
                        else if ((zlevels[k] >= pts[i0, j0].Z && zlevels[k]
        ...
        private void AddContour3D(Graphics g, DataSeries ds,
ChartStyle cs, ChartStyle2D cs2d)
        {
            Pen aPen = new Pen(ds.LineStyle.LineColor, ds.LineStyle.Thickness);
            aPen.DashStyle = ds.LineStyle.Pattern;
                ...
                for (int j = 0; j < pts.GetLength(1) - 1; j++)
                {
                    for (int k = 0; k < numberContours; k++)
                    {
                        ...
                        && zlevels[k] >= pts[i2, j2].Z))
                        {
                            ...
                            pts[i1, j1].X + zratio * pts[i2, j2].X,
                            pts[i1, j1].Y, cs.ZMin, 1);
                            pta[0].Transform(m, form1, cs);
                            pta[1].Transform(m, form1, cs);
                            g.DrawLine(aPen, pta[0].X, pta[0].Y, pta[1].X,
                            pta[1].Y);
                        }
                        else if ((zlevels[k] >= pts[i0, j0].Z && zlevels[k]
                        < pts[i2, j2].Z || zlevels[k] < pts[i0, j0].Z
                        ...
                        && zlevels[k] >= pts[i2, j2].Z))
                        {
                            ...
                            (pts[i2, j2].Z - pts[i0, j0].Z);
                            pta[1] = new Point3(pts[i0, j0].X *
                            (1 - zratio) + pts[i2, j2].X * zratio, pts[i0, j0].Y * (1 - zratio) + pts[i2, j2].Y * zratio, cs.ZMin, 1);
                            pta[0].Transform(m, form1, cs);
                            pta[1].Transform(m, form1, cs);
                            g.DrawLine(aPen, pta[0].X, pta[0].Y, pta[1].X,
                            pta[1].Y);
        ...
        private void AddXYColor3D(Graphics g, DataSeries ds,
ChartStyle cs, ChartStyle2D cs2d)
        {
            Pen aPen = new Pen(ds.LineStyle.LineColor,
            ds.LineStyle.Thickness);
            ...
            if (!IsInterp)
            {
                for (int i = 0; i < pts.GetLength(0) - 1; i++)
                {
                    for (int j = 0; j < pts.GetLength(1) - 1; j++)
                    {
                        ...
                        pta[2] = new PointF(pts1[i + 1, j + 1].X,
                        pts1[i + 1, j + 1].Y);
                        pta[3] = new PointF(pts1[i + 1, j].X,
                        pts1[i + 1, j].Y);
                        Color color = AddColor(cs, pts[i, j], zmin, zmax);
                        aBrush = new SolidBrush(color);
                        g.FillPolygon(aBrush, pta);
...
ChartStyle2D cs2d)
        {
            ...
            if (XYZSlice == SliceEnum.XSlice)
            {
                ...
                int nfix = (int)((SliceLocation - ds.XDataMin) /
                ds.XSpacing) + 1;
                for (int j = 0; j < pts.GetLength(1) - 1; j++)
                {
                    for (int k = 0; k < pts.GetLength(2) - 1; k++)
                    {
                        pta[0] = new PointF(pts[nfix, j, k].point3.X,
                        pts[nfix, j, k].point3.Y);
                        pta[1] = new PointF(pts[nfix, j + 1, k].point3.X,
                        pts[nfix, j + 1, k].point3.Y);
                        pta[2] = new PointF(pts[nfix, j + 1, k + 1].point3.X,
                        pts[nfix, j + 1, k + 1].point3.Y);
                        pta[3] = new PointF(pts[nfix, j, k + 1].point3.X,
                        pts[nfix, j, k + 1].point3.Y);
                        Color color = AddColor(cs, pts[nfix, j, k].point3,
                        vmin, vmax);
                        aBrush = new SolidBrush(color);
                        g.FillPolygon(aBrush, pta);
                        if (ds.LineStyle.IsVisible)
            ...
            else if (XYZSlice == SliceEnum.YSlice)
            {
                ...
                int nfix = (int)((SliceLocation - ds.YDataMin) /
                ds.YSpacing) + 1;
                for (int i = 0; i < pts.GetLength(0) - 1; i++)
                {
                    for (int k = 0; k < pts.GetLength(2) - 1; k++)
                    {
                        pta[0] = new PointF(pts[i, nfix, k].point3.X,
                        pts[i, nfix, k].point3.Y);
                        pta[1] = new PointF(pts[i + 1, nfix, k].point3.X,
                        pts[i + 1, nfix, k].point3.Y);
                        pta[2] = new PointF(pts[i + 1, nfix, k + 1].point3.X,
                        pts[i + 1, nfix, k + 1].point3.Y);
                        pta[3] = new PointF(pts[i, nfix, k + 1].point3.X,
                        pts[i, nfix, k + 1].point3.Y);
            ...
            else if (XYZSlice == SliceEnum.ZSlice)
            {
                ...
                int nfix = (int)((SliceLocation - ds.ZZDataMin) /
                ds.ZSpacing) + 1;
                for (int i = 0; i < pts.GetLength(0) - 1; i++)
                {
                    for (int j = 0; j < pts.GetLength(1) - 1; j++)
                    {
                        pta[0] = new PointF(pts[i, j, nfix].point3.X,
                        pts[i, j, nfix].point3.Y);
                        pta[1] = new PointF(pts[i + 1, j, nfix].point3.X,
                        pts[i + 1, j, nfix].point3.Y);
                        pta[2] = new PointF(pts[i + 1, j + 1, nfix].point3.X,
                        pts[i + 1, j + 1, nfix].point3.Y);
                        pta[3] = new PointF(pts[i, j + 1, nfix].point3.X,
                        pts[i, j + 1, nfix].point3.Y);
                        Color color = AddColor(cs, pts[i, j, nfix].point3,
                        vmin, vmax);
                        aBrush = new SolidBrush(color);
                        g.FillPolygon(aBrush, pta);
                        if (ds.LineStyle.IsVisible)
        ...
        private void AddBar3D(Graphics g, DataSeries ds,
ChartStyle cs, ChartStyle2D cs2d)
        {
            Matrix3 m = Matrix3.AzimuthElevation(cs.Elevation, cs.Azimuth);
        ...
        private void DrawBar(Graphics g, DataSeries ds, ChartStyle cs,
Matrix3 m, Point3 pt, float xlength, float ylength,
float zorign, float zmax, float zmin)
        {
            Pen aPen = new Pen(ds.LineStyle.LineColor, ds.LineStyle.Thickness);
            aPen.DashStyle = ds.LineStyle.Pattern;
            ...
            else if (!IsBarSingleColor && IsColorMap)
            {
                ...
                {
                    z = zorign + i * dz;
                    pt3[0] = new Point3(pts1[nconfigs[0]].X,
                    pts1[nconfigs[0]].Y, z, 1);
                    pt3[1] = new Point3(pts1[nconfigs[1]].X,
                    pts1[nconfigs[1]].Y, z, 1);
                    pt3[2] = new Point3(pts1[nconfigs[1]].X,
                    pts1[nconfigs[1]].Y, z + dz, 1);
                    pt3[3] = new Point3(pts1[nconfigs[0]].X,
pts1[nconfigs[0]].Y, z + dz, 1);
                ...
                pt3[0] = new Point3(pts1[nconfigs[0]].X,
                pts1[nconfigs[0]].Y, zorign, 1);
                pt3[1] = new Point3(pts1[nconfigs[1]].X,
                pts1[nconfigs[1]].Y, zorign, 1);
                pt3[2] = new Point3(pts1[nconfigs[1]].X,
                pts1[nconfigs[1]].Y, pt.Z, 1);
                pt3[3] = new Point3(pts1[nconfigs[0]].X,
                pts1[nconfigs[0]].Y, pt.Z, 1);
                for (int j = 0; j < pt3.Length; j++)
                {
                    pt3[j].Transform(m, form1, cs);
                }
